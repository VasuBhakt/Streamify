...
const LikedVideos = () => {
    ...
    const fetchLikedVideos = useCallback(async (pageNumber = 1) => {
        try {
            setLoading(true);
            const response = await userService.getLikedVideos({
                page: pageNumber,
                limit: 10
            });

            if (response?.data) {
                // The API returns docs that contain the video object
                const mappedVideos = response.data.docs.map(doc => ({
                    ...doc.video,
                    likedAt: doc.createdAt // also get the like timestamp if available
                }));
            ...
            console.error("LikedVideos :: fetchLikedVideos :: error", error);
        } finally {
            setLoading(false);
        }
    }, []);
                ...
                <Container>
                    <div className="px-4">
                        ...
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-xs font-black uppercase tracking-widest text-pink-500">Your Collection</span>
                        </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-surface/30 border-b border-border">
                                    ...
                                    <th className="hidden md:table-cell px-8 py-5 text-xs font-black uppercase tracking-widest text-text-muted">Stats</th>
                                ...
                                {videos.length === 0 ? (
                                    <tr>
                                        <td colSpan="3" className="px-8 py-20 text-center text-text-secondary font-medium italic">
                                            No videos liked yet. Discover something you love!
                                        </td>
                                    </tr>
                                ) : (
                                    videos.map((video) => (
                                        <tr key={video._id} className="hover:bg-surface/20 transition-colors group">
                                            <td className="px-8 py-5 min-w-[300px]">
                                                <div className="flex items-center gap-4">
                                                    <div className="relative w-32 aspect-video rounded-xl overflow-hidden bg-surface-hover shrink-0 border border-border group-hover:border-pink-500 transition-colors">
                                                        <img
                                                            src={video.thumbnail}
                                                            alt={video.title}
                                            ...
                                            <td className="px-8 py-5 text-right">
                                                <Link
                                                    to={`/video/${video._id}`}
                                                    className="inline-flex items-center gap-2 p-2.5 bg-surface text-text-secondary hover:text-pink-500 hover:bg-pink-500/10 rounded-xl border border-border transition-all cursor-pointer group/link"
                                                >
                                                    <span className="text-xs font-bold hidden sm:inline">Watch Now</span>
